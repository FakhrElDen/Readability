...
@section('header')
    ...
    <div class="contentWrapper__right__heading">
        <h1 class="color-black cap-case">Cities</h1>
    </div>
...
@section('content')
    ...
    <div class="contentWrapper__content"><!--Start contentWrapper__content-->
        <div class="contentWrapper__content__form">
            ...
            <form action="{{env('APP_URL')}}/store-city" method="POST">
                {{ @csrf_field() }}
                ...
                <tbody>
                    @forelse($cities as $key=>$city)
                        <tr>
                            ...
                            <td>
                                <span class="color-black cap-case"> {{ $city->name }} </span>
                            </td>
                            <td class="text-right">
                                <a href="{{env('APP_URL')}}/edit-city/{{$city->id}}">
                                    <button class="upper-case color-black">edit</button>
                                </a>
                                <span class="pack__table__separat"></span>
                                <a href="{{env('APP_URL')}}/delete-city/{{$city->id}}">
